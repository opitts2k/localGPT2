...
PDF_DIRECTORY = "./pdfs"
DB_BASE_DIRECTORY = "./DB"  # Base directory for vector stores
DEFAULT_VECTOR_STORE = "SLAVE"  # Default vector store name
...
def get_vector_stores():
    """Get a list of available vector stores in the DB directory"""
    if not os.path.exists(DB_BASE_DIRECTORY):
        os.makedirs(DB_BASE_DIRECTORY)
        return []
    
    # Get subdirectories in the DB directory
    subdirs = [d for d in os.listdir(DB_BASE_DIRECTORY) 
              if os.path.isdir(os.path.join(DB_BASE_DIRECTORY, d))]
...
def create_vector_store(name):
    ...
    store_path = os.path.join(DB_BASE_DIRECTORY, name)
...
class LLMManager:
    ...
    def __init__(self, model_name: str, use_history: bool, vector_store: str = DEFAULT_VECTOR_STORE):
        self.model_name = model_name
        self.use_history = use_history
        self.embedding_model_name = EMBEDDING_MODEL_NAME or model_name
        self.vector_store = vector_store
        self.persist_directory = os.path.join(DB_BASE_DIRECTORY, vector_store)
        
        logger.info(f"Initializing LLM Manager with model: {model_name}, history: {use_history}, vector store: {vector_store}")
    ...
    def _init_qa_pipeline(self):
        ...
        logger.info(f"Connecting to vector database at: {self.persist_directory}")
        
        # Create the directory if it doesn't exist
        if not os.path.exists(self.persist_directory):
            os.makedirs(self.persist_directory)
            logger.warning(f"Created empty vector database directory at {self.persist_directory}")
...
class MultiAgentLLM:
    ...
    def __init__(self, model_name: str, use_history: bool, vector_store: str = DEFAULT_VECTOR_STORE):
        logger.info(f"Initializing MultiAgentLLM with model={model_name}, use_history={use_history}, vector_store={vector_store}")
        
        # Check CUDA availability
        device_type = self._check_cuda_availability()
        
        self.api_clients = APIClientManager(device_type=device_type)
        self.llm_manager = LLMManager(model_name, use_history, vector_store)
